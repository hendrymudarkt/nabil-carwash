<?php
$data = $this->db->query("SELECT max(kode_antrian) as maxKode FROM antrian")->row_array();
$nomor = $data['maxKode'];
$noUrut = (int) substr($nomor, 6, 6);
$noUrut++;
$char = "KODE-";
...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="<?php echo site_url('dist/css/chosen.css');?>">
...
======================================================-->
    <div id="home-p" class="home-p pages-head2 text-center">
      <div class="container">
        <h1 class="wow fadeInUp" data-wow-delay="0.1s">Booking</h1>
      </div><!--/end container-->
    </div> 
        ...
        <div class="row con-form">
          <div class="col-md-12">
            ...
            <?php
                ...
                ?>
            <form action="<?php echo site_url('utama/add') ?>" method="post" id="login-form">
              <div class="row con-form">
                <div class="col-md-4">
                  <input type="text" name="kode_antrian" placeholder="Kode Antrian" class="form-control" value="<?php echo $nomor ?>" readonly>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-6">
                  <input type="text" name="plat" placeholder="Plat Mobil" class="form-control">
                </div><br><br><br>
                <div class="col-md-6">
                  <select name="id_pencuci" class="form-control" id="id_pencuci">
                    <option value="0">Pencuci</option>
                    <?php foreach ($pencuci as $p): ?>
                    <option value="<?php echo $p['id_pencuci'] ?>"><?php echo $p['nama_pencuci'] ?></option>
                    <?php endforeach; ?>
                  </select>
								</div><br><br><br>
								<div class="col-md-12">
                  <select name="id_paket" class="form-control" id="id_paket">
                    ...
                    <option value="<?php echo $r['id_paket'] ?>"><?php echo $r['nama_paket']." (".$r['keterangan'].") "." Harga : Rp. ".number_format($r['harga']) ?></option>
                    <?php endforeach; ?>
                  </select>
                </div><br><br><br>
                <div class="col-md-12 sub-but"><button type="submit" class="btn btn-general btn-white" role="button">Kirim</button></div>
              </div>
            </form>
          </div>
        </div>
...
======================================================-->
    ...
    <script src="<?php echo site_url('dist/js/chosen.jquery.js');?>"></script>
    ...
    <script type="text/javascript">
            $(document).ready(function(){
                $("#no_pam").chosen();
                $('#no_pam').change(function(){
                    var no_pam=$(this).val();
                    $.ajax({
                        url : "<?php echo site_url('pelanggan/get_pelanggan');?>",
                        method : "POST",
                        data : {no_pam: no_pam},
                        async : false,
                        dataType : 'json'
            ...
            $(function() { // code required jquery and image picker. Add those in head section
              $("#id_rambut").imagepicker(); // you can pass param too customize it in your manner
            })
    </script>
  </body>
